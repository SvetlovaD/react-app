...
const dialogsReducer = (state = initialState, action) => {
    ...
    switch (action.type) {
        ...
        case SEND_MESSAGE:
            ...
            console.log(state.dialogs);
            /* let neededID = state.users[action.currentDialogId].messages.slice(-1)[0].id + 1;
            let dispatchTime = new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
            state.users[action.currentDialogId].messages.push({ id: neededID, text: body, dispatchTime: dispatchTime, myMessage: true });
            return state; */
            let dispatchTime = new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
            return {
                ...state,
